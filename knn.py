...
#save model
filename = f'./result/knn/knn{s}.pkl'
...
row = ['K-Nearest Neighbors',"{:.4f}".format(acc),"{:.4f}".format(p),"{:.4f}".format(r),"{:.4f}".format(f1_scr),"{:.4f}".format(auc_precision_recall)]
writer.writerow(row)
file.close()

my_path=format(os.getcwd())
my_path=os.path.join(my_path,'result/knn')
my_file = f'classification_report_knn{s}.txt'  #filename
...
cm_display.plot()
plt.title(f'confusion matrix knn{s}')
my_file = f'confusion_matrix_knn{s}.png' #filename
filename = os.path.join(my_path,my_file)
