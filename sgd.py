...
#save model
filename = f'./result/sgd/sgd_model{s}.pkl'
...
row = ['Stochastic Gradient Descent',"{:.4f}".format(acc),"{:.4f}".format(p),"{:.4f}".format(r),"{:.4f}".format(f1_scr),"{:.4f}".format(auc_precision_recall)]
writer.writerow(row)
file.close()

my_path=format(os.getcwd())
my_path=os.path.join(my_path,'result/sgd')
my_file = f'classification_report_sgd{s}.txt'  #filename
filename = os.path.join(my_path,my_file)

with open(filename,'w') as file:
    file.write(report)
    file.write(f'\nstochastic gradient descent run number : {s}\n')
...
cm_display.plot()
plt.title(f'confusion matrix sgd{s}')
my_file = f'confusion_matrix_sgd{s}.png' #filename
filename = os.path.join(my_path,my_file)
